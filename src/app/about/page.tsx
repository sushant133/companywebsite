...
export default function AboutPage() {
  return (
    <>
      ...
      {/* ---- Hero ---- */}
      <section className="relative flex items-center overflow-hidden bg-[linear-gradient(140deg,#0b1120_0%,#1a1040_45%,#0b1120_100%)] pt-[130px] pb-25 md:min-h-[70vh] md:pt-35 md:pb-30">
            ...
            <Reveal animation="fade-right">
              ...
              <div className="my-9 grid grid-cols-1 gap-5 md:grid-cols-2">
                {aboutHighlights.map((item) => (
                  <div
                    key={item.title}
                    className="flex items-start gap-3.5 rounded-[16px] card-surface p-4 transition-all duration-300 hover:-translate-y-0.5 hover:border-brand/30 hover:shadow-raised"
                  >
          ...
          <div className="grid grid-cols-1 gap-[30px] lg:grid-cols-3">
            {foundationCards.map((card, index) => (
              <Reveal
                key={card.title}
                delay={(index + 1) * 100}
                className="group ring-gradient relative overflow-hidden rounded-[24px] card-surface px-[34px] py-11 transition-all duration-400 hover:-translate-y-1.5 hover:shadow-float"
              >
                <span
                  aria-hidden
                  className={cn(
          ...
          <div className="grid grid-cols-1 gap-[30px] md:grid-cols-2 lg:grid-cols-3">
            {coreValues.map((value, index) => (
              <Reveal
                ...
                className="group ring-gradient relative overflow-hidden rounded-[22px] card-surface px-7 py-9 text-center transition-all duration-300 hover:-translate-y-1.5 hover:shadow-float"
              >
              ...
              <Accordion type="single" collapsible className="gap-3">
                {differentiators.map((item, index) => (
                  <AccordionItem
                    key={item.title}
                    value={`wcp-${index}`}
                    className="group/item mb-3 overflow-hidden rounded-[16px] card-surface transition-all duration-300 data-[state=open]:border-brand/30 data-[state=open]:shadow-raised"
                  >
                ...
                <div className="relative z-[2] grid grid-cols-[repeat(3,70px)] grid-rows-[repeat(3,70px)] gap-2.5 xs:grid-cols-[repeat(3,80px)] xs:grid-rows-[repeat(3,80px)] md:grid-cols-[repeat(3,100px)] md:grid-rows-[repeat(3,100px)] md:gap-[15px]">
                  {hexTiles.map((tile) => (
                    <div
                      ...
                      className="flex size-[70px] flex-col items-center justify-center gap-1.5 rounded-[20px] card-surface transition-all duration-300 animate-float-hex hover:-translate-y-[5px] hover:border-brand/35 hover:shadow-raised xs:size-20 md:size-25"
                    >
          ...
          <div className="grid grid-cols-2 gap-6 md:grid-cols-3 lg:grid-cols-4">
            {industries.map((industry, index) => (
              <Reveal
                ...
                className="group ring-gradient relative rounded-[22px] card-surface px-5 py-9 text-center transition-all duration-300 hover:-translate-y-1.5 hover:shadow-float"
              >
