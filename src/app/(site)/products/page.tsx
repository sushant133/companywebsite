import type { Metadata } from "next";
import { FaCircleCheck } from "react-icons/fa6";

import { CtaSection } from "@/components/site/cta-section";
import { Icon } from "@/components/site/icon";
import {
  Container,
  PageBanner,
  Section,
  SectionHeader,
} from "@/components/site/layout-primitives";
import { Reveal } from "@/components/site/reveal";
import { plainText } from "@/components/site/rich-text";
import { StatsBar, type Stat } from "@/components/site/stats-bar";
import { getContent } from "@/lib/content/store";
import { cn } from "@/lib/utils";

export async function generateMetadata(): Promise<Metadata> {
  const products = await getContent("products");
  return {
    title: "Products",
    description:
      products.banner.description || plainText(products.banner.title),
  };
}

export default async function ProductsPage() {
  const [products, about] = await Promise.all([
    getContent("products"),
    getContent("about"),
  ]);

  /**
   * The banner card carries figures about the catalogue itself rather than the
   * site-wide set, counted off the product and industry content so they cannot
   * drift as the lineup grows.
   */
  const productStats: Stat[] = [
    {
      icon: "FaBoxesStacked",
      value: String(products.items.length),
      label: "Ready-to-Deploy Products",
    },
    {
      icon: "FaListCheck",
      value: String(
        products.items.reduce((n, product) => n + product.features.length, 0),
      ),
      label: "Built-in Features",
    },
    {
      icon: "FaIndustry",
      value: String(about.industries.items.length),
      label: "Industries Covered",
    },
    { icon: "FaHeadset", value: "24/7", label: "Support Included" },
  ];

  return (
    <>
      <PageBanner banner={products.banner} overlap />

      {/* ---- Stats, straddling the banner edge ---- */}
      <StatsBar stats={productStats} />

      <Section>
        <Container>
          <SectionHeader
            tag={products.header.tag}
            title={products.header.title}
            description={products.header.description}
          />

          <div className="grid grid-cols-1 gap-[30px] md:grid-cols-2 lg:grid-cols-3">
            {products.items.map((product, index) => (
              <Reveal
                key={product.title}
                delay={((index % 3) + 1) * 100}
                className="flex flex-col rounded-[20px] border border-slate-200 bg-white px-[30px] py-10 shadow-[0_4px_6px_-1px_rgb(0_0_0_/_0.1),0_2px_4px_-2px_rgb(0_0_0_/_0.1)] transition-all duration-300 hover:-translate-y-2 hover:shadow-brand"
              >
                <div className="mb-5 flex items-start justify-between">
                  <div className="flex size-16 items-center justify-center rounded-2xl bg-brand/10 text-[1.8rem] text-brand">
                    <Icon name={product.icon} />
                  </div>
                  {product.badge ? (
                    <span
                      className={cn(
                        "rounded-full px-3.5 py-1 text-[0.75rem] font-bold tracking-[1px] uppercase",
                        product.featured
                          ? "bg-gradient-brand text-white"
                          : "bg-brand/10 text-brand",
                      )}
                    >
                      {product.badge}
                    </span>
                  ) : null}
                </div>

                <h3 className="mb-3.5 text-[1.35rem]">{product.title}</h3>
                <p className="mb-6 text-[0.95rem] leading-[1.7] text-slate-500">
                  {product.description}
                </p>

                <ul className="flex-1">
                  {product.features.map((feature) => (
                    <li
                      key={feature}
                      className="flex items-center gap-2.5 border-b border-slate-100 py-2 text-[0.95rem] text-ink-3 last:border-b-0"
                    >
                      <FaCircleCheck className="shrink-0 text-[0.9rem] text-brand" />
                      {feature}
                    </li>
                  ))}
                </ul>
              </Reveal>
            ))}
          </div>
        </Container>
      </Section>

      <CtaSection cta={products.cta} />
    </>
  );
}
